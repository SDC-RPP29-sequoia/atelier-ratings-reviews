...
module.exports = (db, filename = 'characteristics.csv') => {
  const {
    Characteristic,
    Product,
    ReviewMetadata } = db;
  ...
  const transformAndLoad = (json, lineNumber) => {
    return new Promise((resolve, reject) => {
      Characteristic.findOne({ where: { characteristic_id: json.id } })
      .then(row => {
        const charateristic = row?.get();
        if (charateristic) {
          resolve(charateristic);
        } else {
          createCharacteristic(json, filename, lineNumber)
          .then(charateristic => resolve(charateristic))
  ...
  const createCharacteristic = (json, filename, lineNumber) => {
    return new Promise((resolve, reject) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      .then(rows => {
        ...
        if (created) {
          ...
          const findMetaDataId = (resolve, reject, limit = 0) => {
            ...
            .then(row => {
              ...
              if (metadata) {
                ...
                console.log(`\nReviewMetadata.findOne failed to find metadata for product ID ${product.id}`);
          ...
          return findMetaDataId(resolve, reject, 5);
        }
      })
      .catch(error => {
