...
describe('DB controller tests', function () {
  beforeAll(() => {
    return new Promise((resolve, reject) => {
      ...
      .then(databases => {
        postgresDB = databases.secondary; //postgres;
        db = databases.methods;
        console.log('db: ', db);
  ...
  afterAll(() => {
    return new Promise((resolve, reject) => {
      if (postgresDB) {
        ...
        .catch(error => {
          console.log('Postgres had an error closing in the controller', error);
          reject(error);
        });
      } else {
  ...
  describe('getReview', function () {
    it('Returns an existing review by public review ID', done => {
      ...
      db.getReview(reviewExpected.review_id)
      .then(review => {
